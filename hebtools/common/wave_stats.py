...
class WaveStats:
    ...
    def get_zero_upcross_periods(self, column_name, hdf5_file_name):
        ...
        print "zero_cross_t_stamps", zero_cross_t_stamps
        df = pd.DataFrame(zero_upcross_periods, 
                          index = zero_cross_t_stamps[:-1])
        df.to_hdf(hdf5_file_name, 'zero_crossing_periods', format='t',
                  append=False, complib='blosc', complevel=9)    
        
    def check_wave_height_dataframe(self, wave_height_df):
        """Gets the maximum standard deviation factor for any wave using the
           indexes from the peaks and adds it to the wave_height_dataframe as
           a column.
        """
        columns = ['max_std_factor']
        ...
        logging.info((len(stats_dict[columns[0]]), len(wave_height_df.index)))
    ...
    def calc_stats(self, column_name, error_check, series_name, hdf5_file_name):
        ...
        p_to_p_period_df.index = wave_height_timestamps[:len(p_to_p_period_df)]
        ...
        wave_height_df.to_hdf(hdf5_file_name, 'wave_height', format='t',
                              append=False, complib='blosc', complevel=9)